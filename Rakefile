...
desc 'Test the hydraulic_brake gem.'
...
namespace :changeling do
  ...
  task :bump, :part do |t, args|
    # Thanks, Jeweler!
    if HydraulicBrake::VERSION  =~ /^(\d+)\.(\d+)\.(\d+)(?:\.(.*?))?$/
      major = $1.to_i
      minor = $2.to_i
      patch = $3.to_i
      build = $4
    ...
    File.open(File.join("lib", "hydraulic_brake", "version.rb"), "w") do |f|
      f.write <<EOF
...
EOF
  ...
  task :change do
    load 'lib/hydraulic_brake/version.rb'
    file    = "CHANGELOG"
    old     = File.read(file)
    version = HydraulicBrake::VERSION
